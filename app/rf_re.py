    ...
    if run_modeling or s_state['modeling_done']:
        if not s_state['modeling_done']:
            try:
                ...
                # Run Random Forest Regression with Bayesian hyperparameter search
                scorer = make_scorer(mean_squared_error, greater_is_better=False)  # Negative MSE for minimization

                opt_rf = BayesSearchCV(
                    RandomForestRegressor(random_state=s_state['parameter_random_state']),
                    hyperparameters,
                ...
                y_true_cv, y_pred_cv = cross_val_regression(x, y, opt_rf.best_estimator_, cv)

                # Statistics for cross-validation
                statistics = calc_statistics(y_true_cv, y_pred_cv)

                # converting calculated metrics into a pandas dataframe to save as xls
                model_type = "Random Forest Regressor"
                result_type = "Cross-Validation"

                metrics_rf_cv = statistics
                metrics_rf_cv['Model'] = model_type
                metrics_rf_cv['Result Type'] = result_type

                st.header('**Metrics of the model on K-fold cross-validation**')

                # Bar chart for Statistics of cross-validation
                metrics_rf_cv_graph = metrics_rf_cv.drop(columns=['Model', 'Result Type'])
                ...
                s_state['metrics_rf_cv'] = metrics_rf_cv
                ...
                def download_model(model):
                    ...
                    href = f'<a href="data:file/output_model;base64,{b64}" download="model_rf.pkl">Download Trained Model (PKL File)</a>'
                    st.markdown(href, unsafe_allow_html=True)

                download_model(opt_rf.best_estimator_)
